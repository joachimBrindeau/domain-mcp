...
npm test               # vitest run (integration tests skipped by default)
npm run test:watch
npm run test:coverage  # thresholds: 80% lines/functions/branches/statements
...
npx vitest run -t "Folder CRUD"   # filter by test name

# Integration / functional / e2e tests (hit the real Dynadot API)
RUN_INTEGRATION_TESTS=true DYNADOT_API_KEY=xxx TEST_DOMAIN=your-domain.com npm test
RUN_INTEGRATION_TESTS=true TEST_DOMAIN=your-domain.com npx vitest run test/e2e.test.ts
RUN_INTEGRATION_TESTS=true TEST_DOMAIN=your-domain.com npx vitest run test/functional.test.ts
```
...
4. **`src/register.ts`** — `registerAllTools` iterates `compositeTools`. For each one it:
   ...
   Then it registers the three standalone tools: `check_domain`, `generate_domain_ideas`, `help` (`src/tools/*.ts`). `generate_domain_ideas` is pure logic on top of `client.execute('search', ...)` — it generates candidates (exact / hyphenated / prefix / suffix patterns) and checks availability in batches of 100 (Dynadot's per-call cap).
