...
4. **`src/register.ts`** — `registerAllTools` iterates `compositeTools`. For each one it:
   ...
   Then it registers the three standalone tools: `domains.availability.check`, `domains.ideas.generate`, `server.help` (`src/tools/*.ts`). `domains.ideas.generate` is deterministic logic on top of `client.execute('search', ...)`: ranked DataForSEO variations take priority over host-supplied LLM fallback variations, each phrase expands to compact and dashed labels for exact/hyphenated patterns, candidates are de-duplicated in stable order, and availability is checked one-at-a-time. Availability checks fail closed on nested Dynadot errors and empty results; never coerce an API failure into `available: false`. Dynadot's `search` command only accepts a single `domain0` per call (any n>1 errors with "too many domains entered") — do not try to reintroduce batching or concurrency.
